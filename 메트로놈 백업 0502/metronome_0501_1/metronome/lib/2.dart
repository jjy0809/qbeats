...
class _MetronomeScreenState extends State<MetronomeScreen> with WidgetsBindingObserver {
  ...
  ClickKind _clickKind = ClickKind.dgit3;
  ...
  Future<void> _loadPrefs() async {
    try {
      ...
      setState(() {
        ...
        if (savedMasterVol != null) _masterVolume = savedMasterVol.clamp(0.0, 1.0);
              ...
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Material(
                        color: cPrimary.withValues(alpha: 1.0),
                                ...
                                Expanded(
                                  child: SliderTheme(
                                    data: SliderTheme.of(context).copyWith(
                                      ...
                                      overlayColor: localTheme.color.withValues(alpha: 0.22),
                                    ...
                                    child: Slider(
                                      min: 0.0,
                                      max: 1.0,
                                      value: localVol.clamp(0.0, 1.0),
                                      onChanged: (v) {
                                        setSheet(() => localVol = v.clamp(0.0, 1.0));
                                      },
                                      onChangeEnd: (v) async {
                                        localVol = v.clamp(0.0, 1.0);
                                        await applyAll(setSheet);
                                      },
                                    ),
                                  ),
                                ...
                                Expanded(
                                  child: SliderTheme(
                                    ...
                                    child: Slider(
                                      ...
                                      onChanged: (v) {
                                        setSheet(() => localBal = v.clamp(-1.0, 1.0));
                                      },
                                      onChangeEnd: (v) async {
                                        localBal = v.clamp(-1.0, 1.0);
                                        await applyAll(setSheet);
                                      },
                                    ),
                                  ),
...
class BeatCircles extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final bigD = cell * 0.75;
    final smallD = cell * 0.60;
    final tinyD = cell * 0.40;
    const gap = 70.0;
    ...
    Widget beatCircle(int index, bool visible) {
      ...
      final double alphaBase = (level == 1) ? 0.7 : (level == 2) ? 0.7 : 0.3;
