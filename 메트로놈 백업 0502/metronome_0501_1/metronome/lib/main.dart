  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: LayoutBuilder(
          builder: (context, cs) {
            // (수정) 기기 너비에 따른 스케일(팝업/메뉴 글자 크기 등에도 반영)
            final scale = (cs.maxWidth / designW).clamp(0.75, 1.40);

            return Stack(
              children: [
                Positioned.fill(
                  child: Container(color: cBg),
                ),

                // (수정) 큰 연회색 배경 패널 제거 (중복 패널 원인)
                // 기존 패널(AnimatedContainer) 하나만 사용
                                ...
                                child: AnimatedContainer(
                                  ...
                                  child: Column(
                                    children: [
                                      ...
                                      SizedBox(height: _showMemoryGrid ? 70 : 150),
                                      Row(
                                        mainAxisAlignment: MainAxisAlignment.center,
                                        children: [
                                          Transform(
                ...
                if (_bannerLoaded && _bannerAd != null)
                  Positioned(
                    ...
                    child: SafeArea(
                      top: false,
                      child: SizedBox(
                        height: (_bannerSize?.height.toDouble() ?? AdSize.banner.height.toDouble()),
                        child: Center(
                          child: SizedBox(
                            width: double.infinity,
                            height: (_bannerSize?.height.toDouble() ?? AdSize.banner.height.toDouble()),
